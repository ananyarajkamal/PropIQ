...
TAXONOMY_KNOWLEDGE_BASE: Dict[RiskCategory, RiskDefinition] = {
    ...
    RiskCategory.DATA_USAGE: RiskDefinition(
        ...
        rule_patterns=[
            r"(?:may|reserves?\s+the\s+right\s+to|rights?\s+to)\s+(?:use|train|utilize).*(?:ai|machine\s+learning|models?)",
            r"(?:train|training)\s+(?:ai|machine\s+learning|models?)\s+(?:using|with|on)\s+(?:customer|your|user)\s+data",
            r"customer\s+grants\s+.*(?:right|license)\s+to\s+.*(?:train|improve)\s+(?:ai|models?)",
            r"aggregate\s+and\s+anonymize\s+data",
            r"use\s+customer\s+data\s+for\s+product\s+improvement",
        ],
        suppression_patterns=[
            r"solely\s+to\s+provide\s+the\s+service",
            r"will\s+(?:not|never)\s+use",
            r"shall\s+(?:not|never)\s+use",
            r"does\s+not\s+use\s+(?:customer|client|your)\s+data",
            r"prohibited\s+from\s+using",
            r"not\s+(?:used|trained)\s+for\s+(?:ai|machine\s+learning|model)",
            r"will\s+(?:not|never)\s+(?:train|be\s+used\s+to\s+train)",
            r"process\s+data\s+solely\s+for\s+customer",
            r"no\s+(?:customer|client|user)\s+data\s+(?:is|will\s+be)\s+used",
            r"will\s+never\s+use\s+your\s+data\s+for\s+ai\s+training",
            r"will\s+not\s+use\s+your\s+data\s+for\s+ai\s+training",
            r"never\s+use\s+(?:customer|your)\s+data",
        ],
